export default {
  ...
  methods: {
    ...
    exportFields() {
      return this.exportColumns.reduce((result, column) => {
    ...
    exportData(rows, full, parent = "") {
      return rows.reduce((exportRows, row, index) => {
        ...
        return exportRows.concat([
          ...
          ...(full
            ? row && row._children
              ? this.exportData(row._children, full, order + "-")
              : []
            : row && row._showChildren
            ? this.exportData(row._meta.visibleChildren, full, order + "-")
            : []),
        ]);
      }, []);
    },
