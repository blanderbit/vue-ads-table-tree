export default {
  ...
  computed: {
    ...
    indexesToLoad() {
      ...
      return Array.from(paginatedRows)
        .map((row, index) =>
          row === undefined ? index + this.start : undefined
        )
        .filter((index) => index);
    },
  },
  ...
  methods: {
    noChildrenLoaded(row) {
      return (
        Object.prototype.hasOwnProperty.call(row, "_hasChildren") &&
        row._hasChildren
      );
    },
    ...
    async handleUnresolved() {
      ...
      rows.splice(this.start, this.end, ...result.rows);
      this.tempRows = rows;
    ...
    async loadRows() {
      ...
      this.$set(this.rows, this.indexesToLoad[key], rows[key]);
      this.loading = false;
    },
  },
};
